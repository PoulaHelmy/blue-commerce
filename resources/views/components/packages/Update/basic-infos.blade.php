<div class="row">
    ...
    <div class="form-group col-4">
        ...
        <input type="number"
               ...
               min="0"
               id="travellers"
               value="{{$package->travellers}}"
               required
               placeholder="Package Travellers ">
    ...
    <div class="form-group col-4">
        ...
        <input type="number" class="form-control {{$errors->has('from') ? 'is-invalid' : ''}}"
               ...
               value="{{$package->from}}"
               required
               placeholder="Package Prices Starting From... ">
        @if($errors->has('from'))
    ...
    <div class="form-group col-12">
        ...
        <input type="text" class="form-control {{$errors->has('label') ? 'is-invalid' : ''}}"
               ...
               value="{{$package->label}}"
               required
               placeholder="Package label For Offers,Featured etc... ">
        @if($errors->has('label'))
    ...
    <div class="form-group col-12">
        ...
        <select class="select2" multiple="multiple" name="categories[]"
            ...
            @foreach(\App\Models\Category::where('status',1)->get() as $category)
                <option
                        value="{{$category->id}}" {{ $package->categoriables->contains('category_id', $category->id) ? 'selected' : '' }} >{{$category->name}}</option>
            @endforeach
        </select>
    ...
    {{--                            style="width: 100%;">--}}
    {{--                        @foreach(\App\Models\Package::where(['status'=>1,'is_excursion'=>0])->get() as $Package)--}}
    {{--                            <option--}}
