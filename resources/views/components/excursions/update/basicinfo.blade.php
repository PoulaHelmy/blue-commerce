...
<!---- Travellers --->
    ...
    <div class="form-group col-8">
        ...
        <select class="select2" name="destination_id" data-placeholder="Select Destination"
            ...
            @foreach(\App\Models\Destination::where('status',1)->get() as $destination)
                <option value="{{$destination->id}}" {{$excursion->destination_id == $destination->id ? 'selected':''}}>{{$destination->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-12">
        ...
        <select class="select2" multiple="multiple" name="categories[]"
                data-placeholder="Select Excursion Categoires" style="width: 100%;">
            @foreach(\App\Models\Category::where('status',1)->get() as $category)
                <option
                        value="{{$category->id}}" {{ $excursion->categories->contains($category->id) ? 'selected' : '' }} >{{$category->name}}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-12">
        ...
        <div class="float-right">
            ...
            <input type="checkbox" class="ml-5" name="status"
                   {{$excursion->status == 1 ? 'checked':''}}
                   data-bootstrap-switch
                   data-off-color="danger"
                   data-on-color="success">
        </div>
